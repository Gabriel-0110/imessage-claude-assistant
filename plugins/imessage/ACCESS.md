...
These tools never send and never mutate access state. They respect the
allowlist scope — non-allowlisted chats are invisible to them.

| Tool | Purpose |
| --- | --- |
| `chat_messages` | Full recent history per allowlisted chat |
| `recent_chats` | Per-thread activity overview with unreplied tag |
| `pending_replies` | Threads whose last message is inbound and unanswered |
| `thread_summary` | Stats + recent messages + per-contact style notes |
| `style_profile` | Global + per-contact style and approved-reply examples |
| `health_check` | Self-diagnostic: DB, state, policy, watermark |

## Style learning

`record_approved_reply` appends to
`~/.claude/channels/imessage/style/approved-examples.jsonl` when the
operator explicitly approves a reply in a terminal session. The log is
append-only; entries carry `{ts, contact, decision, final_text, options,
chosen_index, note}`.

Per-contact free-form notes live at
`~/.claude/channels/imessage/style/contacts/<handle>.md` and are read by
`thread_summary` and `style_profile`. Edit them by hand or let the
`record_approved_reply` tool append stamped notes with its optional
`note` argument.

Explicit operator preferences (default tone, per-contact signature overrides)
live at `~/.claude/channels/imessage/style/preferences.json`. The server
reads but never writes this file.
