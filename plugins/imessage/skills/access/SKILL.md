...
here — it would autoreply a code to every contact who texts. `disabled`
...
3. Confirm with the actual meaning of the selected mode:
   - `pairing`: unknown DMs get a pairing code and require approval
   - `allowlist`: only allowlisted DMs are delivered
   - `disabled`: all DMs are delivered without approval or pairing
...
- When showing status or confirming a policy change, describe `disabled` as delivering all DMs without approval or pairing.
