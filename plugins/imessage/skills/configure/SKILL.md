...
3. **What next** — end with a concrete next step based on state:
   ...
   - FDA granted, policy is disabled → *"Ready. All DMs are delivered without
     approval or pairing."*
...
7. **User asks for all senders** → explain that `disabled` now delivers all
   DMs without approval or pairing, and confirm they understand the safety
   tradeoff before recommending it.

Handles are `+15551234567` or `someone@icloud.com`. `disabled` delivers all
DMs without approval or pairing.
...
- When describing policy modes, `disabled` means all DMs are delivered
  without approval or pairing.
